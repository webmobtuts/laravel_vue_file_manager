...
const app_global = inject('app_global', {});
    ...
    const process = () => {
        ...
        axios.post(`/api/${props.action === 'copy' ? 'cp-file':'mv-file'}`, {
...
<template>
    ...
    <a class="nav-link d-flex gap-1 align-items-center" href="#" :title="action === 'copy'?'Action':'Move'" @click.prevent="toggleModal()" :class="{disabled}">
        <slot />
    </a>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">{{action === 'copy' ? 'Copy' : 'Move'}} File</h5>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" @click="process">{{action === 'copy' ? 'Copy' : 'Move'}}</button>
                    </div>
                </div>
            </div>
        </div>
